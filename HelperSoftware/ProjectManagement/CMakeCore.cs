...
namespace CMakeCore
{
    public static class CMakeUtility
    {
        ...
        public static string SetSourceGroup(string projectDirectory, List<FileInfo> codeFiles)
        {
            ...
            {
                data += "source_group(" + groupPair.Key + " FILES " + groupPair.Value + ")\n\n";
            }
        ...
        public static string SetLibraryProjectDefine()
        {
            return "add_definitions(-D_EDITABLE_PROJECT)\n";
        }
    ...
    public class ProjectProperty
    {
        ...
        public HashSet<string> SourcePublicIncludePath { get; protected set; }
        ...
        public ProjectProperty(FileInfo targetFile, ProjectCMakeSetting settingClass)
        {
            ...
            List<string> extensions = new List<string>() { ".cpp", ".h", ".cs" };
            ...
            SourcePublicIncludePath = new HashSet<string>();
            ...
            foreach (var fi in SourceFiles)
            {
                ...
                if (fi.Extension == ".h")
                {
                    ...
                    string relativePath = CMakeUtility.GetRelativePath(ProjectDirectory.FullName, fi.Directory.FullName, '\\');

                    if (relativePath.IndexOf("Public") > -1)
                        SourcePublicIncludePath.Add(fileDirectory);
                }
        ...
        public void ReferencingOtherProject(List<ProjectProperty> allProjectProperties)
        {
            ...
            {
                foreach (var dir in property.SourcePublicIncludePath)
                {
                    SourceIncludePath.Add(dir);
                }
            }
        ...
        public void CreateCMakeFile()
        {
            ...
            data += SettingClass.IsLibrary ? CMakeUtility.SetLibraryProjectDefine() : "";
    ...
    {
        public static void CreateCMakeList(string sourcePath)
        {
            List<FileInfo> projectFiles = new List<FileInfo>();

            DirectoryInfo startPathDI = new DirectoryInfo(sourcePath);
            ...
            for (int i = 0; i < projectFiles.Count; ++i)
            {
                ...
                {
                    usedProjectPaths.Add(CMakeUtility.GetRelativePath(sourcePath, projectFiles[i].Directory.FullName));
            ...
            data += CMakeUtility.SetStartProject(sourcePath, solutionClass.MainProjectName);

            File.WriteAllText(sourcePath + "\\CMakeLists.txt", data);
        }


        static List<ProjectCMakeSetting> GetProjectCMakeSettingClass(FileInfo solutionFile, List<FileInfo> csFiles, out SolutionCMakeSetting solutionClass)
        {
        ...
        static void FindCSFiles(DirectoryInfo current, List<FileInfo> csfiles)
        {
            DirectoryInfo[] dInfos = current.GetDirectories();
